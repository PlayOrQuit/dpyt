...
//    }
    ...
    public function getChannels($token, $optionalParams = false, $part = ['id', 'snippet', 'contentDetails', 'statistics', 'invideoPromotion'])
    {
        $API_URL = $this->getApi('channels.list');
        $params = [
            'mine' => 'true',
        ...
        $apiData = $this->api_get_header($API_URL, $params, $headers);
        if (is_array($apiData)) {
            return $this->decodeMultiple($apiData);
        }
        return $this->decodeSingle($apiData);
    ...
    public function api_get_header($url, $params, $headers)
    {
        $tuCurl = curl_init();
        if($headers != null)
